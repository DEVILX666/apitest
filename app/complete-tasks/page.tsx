...
const CONFIG = {
  apiEndpoint:
    "https://d2jgih9urxpa47.cloudfront.net/public/offers/feed.php?user_id=705486&api_key=3f77ccd539495b1de5d31ccf19649e3a&s1=&s2=",
  requiredTasks: 2,
  debug: true,
}

// Set initial offer percentages
const fixedPercentages = [23, 34, 37, 50]

// Badge color: under 50% = green, 50% and above = orange
...
export default function CompleteTasksPage() {
  ...
  useEffect(() => {
    ...
    const showRandomNotification = () => {
      ...
      // When popup notification shows, increase each offer's percentage by 2%, max 99%
      setOffers((prev) => {
        if (!prev || prev.length === 0) return prev
        return prev.map((offer) => ({
          ...offer,
          percentage: Math.min((offer.percentage || 0) + 2, 99),
        }))
      })
    ...
    const liveInterval = setInterval(() => {
      setLiveCount((prev: number) => Math.max(10, Math.min(25, prev + Math.floor(Math.random() * 3) - 1)))
      setCompletedToday((prev: number) => prev + Math.floor(Math.random() * 3))
    }, 30000)
  ...
  const initializeApp = async () => {
    ...
    await fetchAdBlueMediaOffers(ip)
  }
  ...
  // Fetch AdBlueMedia Offers
  const fetchAdBlueMediaOffers = async (ip: string) => {
    setIsLoading(true)
    setError(null)
    try {
      const url = CONFIG.apiEndpoint
      if (CONFIG.debug) console.log("🎯 Fetching AdBlueMedia offers...", { url })
      const response = await fetch(url)
      const data = await response.json()
      if (CONFIG.debug) console.log("📦 AdBlueMedia API Response:", data)
      if (Array.isArray(data) && data.length > 0) {
        const formattedOffers = data.slice(0, CONFIG.requiredTasks * 2).map((offer: any, idx: number) => {
          let instructions =
            offer.conversion ||
            offer.instructions ||
            ...
            "Follow all instructions carefully to complete this offer and unlock your content."
          return {
            id: offer.id || offer.offer_id || Math.random(),
            title: offer.name || offer.title || "Untitled Offer",
            description: typeof offer.description === "string" ? offer.description : (offer.description?.en || "Complete this offer to unlock your content"),
            reward: offer.payout ? `$${offer.payout} Reward` : "Reward",
            instructions,
            url: offer.url || offer.tracking_url || offer.link,
            image: offer.image_url || offer.image || offer.icon || "/placeholder.svg",
            category: offer.category || "other",
            payout: Number.parseFloat(offer.payout) || 0,
            percentage: fixedPercentages[idx] !== undefined ? fixedPercentages[idx] : 50,
            completedToday: Math.floor(Math.random() * 180) + 120, // 120-300
          }
        })
        setOffers((prev: OGAdsOffer[]) => {
          const updated = [...prev, ...formattedOffers]
          if (updated.length > CONFIG.requiredTasks * 2) updated.pop()
          return updated
        })
        console.log(`✅ Successfully loaded ${formattedOffers.length} AdBlueMedia offers`)
      } else {
        throw new Error("No offers available")
      }
    } catch (error: any) {
      console.error("❌ AdBlueMedia fetch failed:", error)
  ...
  const handleOfferClick = (offer: OGAdsOffer) => {
    ...
    if (!completedOffers.includes(offer.id)) {
      setCompletedOffers((prev: number[]) => {
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              ...
              <div className="bg-white rounded-xl p-6 shadow-lg mb-8">
                ...
                {isLoading ? (
                  <div className="flex flex-col justify-center items-center py-12">
                    ...
                    <p className="text-gray-600">🎯 Connecting to AdBlueMedia API...</p>
                  ...
                  <div className="text-center py-8">
                    ...
                    <button
                      ...
                      onClick={() => fetchAdBlueMediaOffers(userIP)}
                      onMouseEnter={playClickSound}
                    >
                      <RefreshCw className="w-4 h-4" />
                      Retry AdBlueMedia API
                    </button>
                  </div>
                ) : (
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                {typeof offer.percentage === 'number' && (
                                  <span
                                    className={`text-xs px-2 py-1 rounded-full text-white ${getPercentageColor(offer.percentage)}`}
                                  >
                                    {offer.percentage}%
                                  </span>
                                )}
                              </div>
                              <p className="text-xs text-amber-600 font-bold">{offer.completedToday} TODAY COMPLETED</p>
                            </div>
                          </div>
              ...
              {/* Proof Section */}
              {/* Removed proof screenshot section */}
              ...
              <div className="bg-white rounded-xl p-6 shadow-lg mb-8">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-gray-600">✅ Yes. Works with any Roblox account, mobile or PC. Just enter your real username. No password needed at all.</p>
                  </div>
                  <div>
                    <p className="font-medium">Q2: Is this actually free?</p>
                    <p className="text-gray-600">🆓 Yes, it’s 100% free. You just need to complete one quick task that takes under a minute to unlock your pet.</p>
                  </div>
                  <div>
                    <p className="font-medium">Q3: Do I need to log in or give my password?</p>
                    <p className="text-gray-600">🚫 Never. We don’t ask for logins or passwords. Only your Roblox username is needed to send the pet.</p>
                  </div>
                  <div>
                    <p className="font-medium">Q4: Why do I have to complete a task?</p>
                    <p className="text-gray-600">🔐 It stops bots and fake users. Only real players who complete a task get access. That’s how we keep pets undetected and working without patches.</p>
                  </div>
                  <div>
                    <p className="font-medium">Q5: I finished the task but didn’t get the pet. What now?</p>
                    <p className="text-gray-600">⏳ You probably skipped a step or closed the tab too early. Go back and complete the task fully. It usually takes less than 2 minutes total.</p>
                  </div>
                </div>
              </div>
